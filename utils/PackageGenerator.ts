...
class PackageGenerator<T extends PackageGeneratorOptions = PackageGeneratorOptions> extends BaseGenerator<T> {
    ...
    configureAnsible(templatePath: string, context: TemplateData = {}): void {
        ...
        if (this.templateExists(`${templatePath}/group_vars.yaml.ejs`)) {
            this.appendTemplate(`${templatePath}/group_vars.yaml.ejs`, `infra/envs/staging/group_vars/inventory/static.yaml`, context);
        }
